...
$confirmPassword = validate_input($_POST['confirm_password']);
	...
	if (!$connection) {
		...
		$sql = "SELECT email_address,password FROM doctor where email_address = '$email' and password = '$currentpassword'";
		$query = mysqli_query($connection, $sql);
		
		if (mysqli_num_rows($query)<=0) {//if the user is not registered
			echo "You are not registered! please register first! ";
			print("<br><a href='docsign.php'>Goto Register</a>");
		} else {
			if ($newpassword != $confirmPassword) {//if password is not matched
				echo "Please confirm your password ! ";
				print("<br><a href='changepassword.php'>Back to Reset Password</a>");
			} else {//updates user password based on user name
				$sqlUpdate = "UPDATE doctor SET Password = '$newpassword' where email_address = '$email'";
